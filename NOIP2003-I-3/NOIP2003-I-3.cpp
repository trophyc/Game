// NOIP2003-I-[]3.cpp : Defines the entry point for the console application.
//
...
errno_t ReadFromFile(FILE* file, int* count, int* segments, int v[]);
...
int main(int argc, char* argv[])
{
   ...
   FILE* file;
   errno_t err = fopen_s(&file, argv[1], "rt");
   if (err) {
   ...
   int v[MAX_COUNT*2];
   int count, segments;
   err = ReadFromFile(file, &count, &segments, v);
   if (err != 0) {
...
void PrintReadIns(int count, int segments, int v[])
{
   printf_s("Total %d numbers should be divided into %d segments.\n", count, segments);
   for (int i = 0; i < count; i++) {
      printf_s("%d ", v[i]);
   }
   printf_s("\n");
